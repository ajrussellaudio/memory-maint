...
const { detectConflicts, formatConflictsMarkdown } = require('./conflicts');
...
/**
 * Run conflict detection on daily logs
 * 
 * @param {Object} options - Detection options
 * @param {number} options.days - Number of days to analyze (default: 14)
 * @param {string} options.skillPath - Path to memory-maint skill directory
 * @param {Function} options.sessions_spawn - OpenClaw sessions_spawn function
 * @param {Function} options.logger - Optional logging function
 * @returns {Promise<Object>} - Result with conflicts array
 */
async function runConflictDetection({ days = 14, skillPath, sessions_spawn, logger = console.log }) {
  logger('🔍 Memory Maintenance - Conflict Detection\n');
  logger(`Analyzing last ${days} days...\n`);
